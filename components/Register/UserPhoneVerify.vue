<template>
  <v-form method="POST" @submit.prevent="submit" ref="form">
    <v-row justify="center" style="margin-top: 100px">
      ...
      <v-col cols="4" md="2">
        <v-btn @click="send_verify_code" class="button-fill"
          >ارسال کد تاييد</v-btn
        >
      </v-col>
    </v-row>
...
export default {
  ...
  methods: {
    ...
    async send_verify_code() {
      try {
        ...
        let res = await this.$axios.$post(
          "phone_number_verify/send_token/",
          this.userData
        );
        this.is_send = true;
      } catch (e) {
    ...
    async submit() {
      if (this.is_send) return await this.verify_code();
    ...
    async verify_code() {
      ...
      try {
        let res = await this.$axios.$post(
          "phone_number_verify/verify_token/",
          this.userData
        );
        this.goToNextPage();
      } catch (e) {
