            ...
            <v-row>
              <v-col>
                <v-card class="cardShape"
                        style="margin-left: 2rem; margin-right: 2rem;margin-top: 2rem;margin-bottom: 2rem">
                  <v-row> <!-- 1 -->
                    <v-col>
                  ...
                  <v-row> <!-- 2 -->
                    <v-col>
                      <p style="margin-top: -2rem" class="mainText">زمان کوییز: <span class="innerText">۵ دقیقه</span>
                      </p>
                    </v-col>
                  </v-row>
                  <v-row> <!-- 3 -->
                    <v-col>
                      <p style="margin-top: -2rem" class="mainText">تعداد سوالات: <span class="innerText">۵ سوال</span>
                      </p>
                    </v-col>
                  </v-row>
                  <v-row> <!-- 4 -->
...
export default {
  ...
  methods: {
    async participate() {
      ...
      try {
        let data = await this.$axios.$post('/quiz/create/')
        this.$router.push('/forms/quizForm/' + data.pk)
      } catch (e) {
        this.$notify({
          group: 'foo',
          type: 'error',
